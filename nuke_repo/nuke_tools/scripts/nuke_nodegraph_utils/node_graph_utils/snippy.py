...
from node_graph_utils.dag import (
    get_current_dag,
    get_node_bounds,
    get_dag_node,
    node_center,
...
class SnippingWidget(QtWidgets.QWidget):
    def __init__(self, dag_widget):
        ...
        self.image = QtGui.QImage(
            dag_rect.size(), QtGui.QImage.Format_ARGB32_Premultiplied
        )
        self.nodes_image = QtGui.QImage(
            dag_rect.size(), QtGui.QImage.Format_ARGB32_Premultiplied
        )
        self.drawing = False
        self.last_pos = None
        ...
        with self.dag_node:
            ...
            ) / scale - QtCore.QPoint(*nuke.center())
    ...
    def get_all_connections(self):
        ...
        painter.setBrush(QtGui.QBrush(my_pen_color, QtCore.Qt.SolidPattern))
        ...
        for node in nuke.allNodes():
            ...
            for i in range(node.inputs()):
                ...
                if input_node and input_node in dependencies:
                    line = QtCore.QLineF(
                        rect.center(), QtCore.QPoint(*node_center(input_node))
                    )
