...
def get_nodes_in_bounds(bounds, include_overlapping=False):
    ...
    if include_overlapping:
        return [
            node for node in nuke.allNodes() if get_node_bounds(node).intersects(bounds)
        ]
...
def sort_nodes_by_position(nodes, axis=0, reverse=False):
    ...
    return sorted(
        nodes,
        key=lambda n: (node_center(n)[axis], node_center(n)[1 - axis]),
        reverse=reverse,
    )
...
def de_intersect():
    ...
    try:
        ...
        while intersecting and loop < max_loops:
            ...
            loop += 1
            progress.setMessage(
                'resolving intersections pass {}/{}(max)'.format(loop, max_loops)
            )
